...
export async function GET() {
  ...
  const [rows] = await pool.query('SELECT * FROM agents WHERE userId = ?', [session.user?.id])
...
export async function POST(req: NextRequest) {
  ...
  // Deduct tokens
  await pool.query('UPDATE users SET tokens = tokens - ? WHERE id = ?', [agentTokenCost, session.user?.id]);
